...
module.exports = function(babel) {
  ...
  function collectStyleDeclarator(path, tagName) {
    ...
    styleMap[key] = value
    binding.path.remove()
    if (isEmptyDeclaration(binding.path.parent)) {
      binding.path.parent.remove()
    }
    return value
  }
      ...
      JSXElement: {
        enter(path) {
          ...
          if (openingElement && isStyledJSXElement(openingElement)) {
            ...
            let style = styleMap[styledTagName]
            if (!style) {
              style = collectStyleDeclarator(path, styledTagName)
            }
